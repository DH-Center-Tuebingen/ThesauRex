...
<template>
    <div :ref="el => nodeRef = el" @dragenter="onDragEnter" @dragleave="onDragLeave"
        ...
        <a href="" :id="`${data.tree}-tree-node-cm-toggle-${data.id}`" @click.prevent
            @contextmenu.stop.prevent="openContextMenu()" class="text-body text-decoration-none disabled"
            data-bs-toggle="dropdown" :data-path="join(data.path)">
...
export default {
    ...
    setup(props, context) {
        ...
        const state = reactive({
            ...
            asyncToggle: computed(_ => _debounce(() => toggleTreeNode(props.data, props.data.tree), 500)),
