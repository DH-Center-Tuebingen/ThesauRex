<template>
    <div class="d-flex flex-column">
        ...
        <div class="d-flex flex-column col px-0 scroll-y-auto">
            <tree v-if="treeData.length > 0" :id="state.treeId" :data="treeData" :draggable="state.dragAllowed"
                :drag-target="dragTarget" :drop-allowed="dropAllowed" size="small" @change="itemClick" @drop="itemDrop"
                @toggle="itemToggle" @change-drag-target="changeDragTarget">
            </tree>
            <div class="h-100 w-100 d-flex align-items-center justify-content-center bg-warning bg-opacity-10 rounded-3 border-dashed border-2 border-secondary mt-2"
                v-else>
        ...
        <div class="position-absolute top-0 start-0 h-100 w-100 bg-light bg-opacity-50" style="z-index: 9999;"
